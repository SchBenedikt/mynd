...
class TestEngineNotifications:
    ...
    def test_notification_ids_unique_within_same_millisecond(self, engine):
        ids = set()
        for i in range(5):
            n = engine.add_notification(f"burst-{i}")
            ids.add(n["id"])
        assert len(ids) == 5
        assert len({n["id"] for n in engine.load_notifications()}) == 5
