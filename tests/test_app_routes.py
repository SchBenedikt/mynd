    ...
    def test_refresh_rotates_token(self, client, monkeypatch, tmp_path):
        ...
        monkeypatch.setitem(
            ...
            'refresh-user',
            {
                'name': 'Refresh User',
                'role': 'user',
                'token_hash': app_state.token_hash('old-token'),
                'token_expires_at': time.time() + 3600,
            },
        )
        ...
        assert (
            client.get('/api/auth/me', headers={'Authorization': 'Bearer old-token'}).get_json()['authenticated']
            is False
        )
        assert (
            client.get('/api/auth/me', headers={'Authorization': f'Bearer {new_token}'}).get_json()['authenticated']
            is True
        )
...
class TestToolFallbackParser:
    ...
    def test_browser_tool_code_is_parsed(self):
        ...
        assert _parse_tool_code_fallback(text) == [{'name': 'browser_open', 'args': {'url': 'https://example.com'}}]
...
class TestSecurityAPI:
    ...
    def test_api_allows_configured_browser_origin_with_security_headers(self, client, monkeypatch):
...
class TestVaultAPI:
    ...
    def test_vault_set_and_delete(self, client):
        resp = client.post(
            '/api/vault/entries', json={'key': 'test/foo', 'value': 'bar'}, content_type='application/json'
        )
...
class TestBriefingAPI:
    ...
    def test_greeting_cached_per_period(self, client, monkeypatch):
        ...
        monkeypatch.setattr(
            'core.llm.chat_with_tools', lambda *a, **k: (calls.append(1), {'message': {'content': 'Hallo'}})[1]
        )
    ...
    def test_gather_briefing_filters_completed_tasks(self, monkeypatch):
        monkeypatch.setattr(app_routes._nc_module, 'nextcloud_caldav_query', lambda *a, **k: ('10:00 Meeting', None))
        monkeypatch.setattr(app_routes._email_module, '_list_accounts', lambda: 'Keine E-Mail-Konten konfiguriert.')
        monkeypatch.setattr(
            app_routes._nc_module,
            'nextcloud_tasks_query',
            lambda: (
                '📌 [Schule] Offen | Fällig: ? | Status: NEEDS-ACTION\n📌 [Schule] Alt | Fällig: ? | Status: COMPLETED'
            ),
        )
        monkeypatch.setattr(
            app_routes._immich_module, 'immich_search_photos', lambda *a, **k: ('(keine Ergebnisse)', None)
        )
...
class TestImmichAPI:
    def test_immich_config_missing_key(self, client):
        resp = client.post('/api/immich/config', json={'url': ''}, content_type='application/json')
    ...
    def test_backup_import_rejects_path_traversal(self, client, monkeypatch, tmp_path):
        ...
        monkeypatch.setitem(
            ...
            'admin',
            {
                'name': 'Admin',
                'role': 'admin',
                'token_hash': app_state.token_hash('test-admin-token'),
                'token_expires_at': time.time() + 3600,
            },
        )
...
def _load_main_script():
    ...
    spec = importlib.util.spec_from_file_location('mynd_main_script', Path(__file__).resolve().parents[1] / 'app.py')
