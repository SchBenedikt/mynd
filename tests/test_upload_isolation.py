...
def _client(username, token):
    ...
    app_module.AUTH_USERS[username] = {
        'name': username,
        'role': 'user',
        'token_hash': state.token_hash(token),
...
def test_uploads_are_randomized_and_isolated(monkeypatch, tmp_path):
    ...
    try:
        first = alice.post(
            '/api/upload', data={'file': (io.BytesIO(b'alice'), 'report.txt')}, content_type='multipart/form-data'
        )
        second = alice.post(
            '/api/upload', data={'file': (io.BytesIO(b'alice-2'), 'report.txt')}, content_type='multipart/form-data'
        )
        ...
        assert re.fullmatch(r'[0-9a-f]{32}', first_data['object_id'])
...
def test_upload_requires_authentication(monkeypatch, tmp_path):
    ...
    with app.test_client() as client:
        response = client.post(
            '/api/upload', data={'file': (io.BytesIO(b'x'), 'x.txt')}, content_type='multipart/form-data'
        )
