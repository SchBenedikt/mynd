...
class KnowledgeBase:
    ...
    def _semantic_search(self, query, k, filter_source=None):
        try:
            ...
            scores = np.array(
                [float(np.dot(qe, e) / (np.linalg.norm(qe) * np.linalg.norm(e) + 1e-10)) for e in self.embs]
            )
...
def _reset_app_data():
    ...
    AI_CONFIG_FILE.write_text(
        json.dumps({'base_url': 'http://127.0.0.1:11434', 'model': '', 'embedding_model': ''}, indent=2)
    )
...
def _save_memory(m):
    ...
    fd, temporary_name = tempfile.mkstemp(prefix='.memory.', dir=DATA_DIR)
    try:
