...
if __name__ == '__main__':
    ...
    # Prüfe auf --no-menu Flag
    if '--no-menu' in sys.argv:
        # Direkter Server-Start ohne Menü
        print("AI-Chat-Anwendung wird gestartet (Menü übersprungen)...")
        ...
        def print_header():
            """Gibt den Header für das CLI-Menü aus"""
            print("\n" + "="*60)
            print("  AI Chat Assistant - Terminal Steuerung")
            print("="*60)

        def print_menu():
            """Gibt das Hauptmenü aus"""
            print_header()
            print("\n  [1] Web-Server starten (Flask)")
            print("  [2] Training Status anzeigen")
            print("  [3] Neues Training starten (simuliert)")
            print("  [4] Knowledge Base Statistiken")
            print("  [5] Ollama Verbindung testen")
            print("  [6] Letzte Interaktionen anzeigen")
            print("  [7] Modell-Info anzeigen")
            print("  [8] Knowledge Base neu laden")
            print("  [0] Beenden")
            print("\n" + "-"*60)
        ...
        def show_training_status():
            ...
            try:
                stats = training_manager.get_training_stats()
                print(f"  Gesamt-Interaktionen: {stats.get('total_interactions', 0)}")
                print(f"  Durchschnittliche Kontext-Quellen: {stats.get('avg_context_sources', 0)}")
                print(f"  Durchschnittliche Antwort-Länge: {stats.get('avg_response_length', 0)} Zeichen")
                print(f"  Dateigröße: {stats.get('training_file_size', 0)} bytes")
                
                if stats.get('most_common_queries'):
                    print("\n  Häufigste Queries:")
                    for query, count in stats.get('most_common_queries', [])[:3]:
                        print(f"    - {query[:50]}... ({count}x)")
        ...
        def show_knowledge_stats():
            ...
            try:
                if knowledge_base.search_engine:
                    ...
                    print(f"  Index-Größe: {stats.get('index_size', 0)}")
                    print(f"  Modell: {stats.get('model_name', 'Unknown')}")
                    print(f"  Datenbank: {stats.get('db_path', 'N/A')}")
                else:
                    print("  Search Engine: Nicht verfügbar")
                    print(f"  Memory Chunks: {len(knowledge_base.knowledge_chunks)}")
            except Exception as e:
                print(f"  Fehler: {str(e)}")
            
            print("\n  " + "-"*40)
            print(f"  Knowledge Datei: {knowledge_base.knowledge_file}")
            print(f"  Existiert: {os.path.exists(knowledge_base.knowledge_file)}")
        ...
        def test_ollama_connection():
            ...
            try:
                ...
                if ollama_client.check_connection():
                    ...
                    # Versuche, verfügbare Modelle zu laden
                    try:
                    ...
                    print("  Tipps:")
                    print("    1. Ist Ollama installiert?")
                    print("    2. Läuft Ollama? (ollama serve)")
                    print(f"    3. Ist die URL korrekt? ({ollama_client.base_url})")
            except Exception as e:
                print(f"  Fehler: {str(e)}")
        ...
        def show_recent_interactions():
            """Zeigt die letzten Interaktionen"""
            print_header()
            print("\n  LETZTE INTERAKTIONEN")
            print("  " + "-"*40)
            
            try:
                if os.path.exists(training_manager.training_data_path):
                    with open(training_manager.training_data_path, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                    ...
                    if not data:
                        ...
                        recent = data[-5:]
                        print(f"  Zeige letzte {len(recent)} von {len(data)} Interaktionen:\n")
                        
                        for i, interaction in enumerate(reversed(recent), 1):
                            query = interaction.get('query', 'N/A')
                            timestamp = interaction.get('timestamp', 0)
                            date_str = datetime.fromtimestamp(timestamp).strftime('%d.%m.%Y %H:%M') if timestamp else 'Unbekannt'
                            sources = interaction.get('context_sources', 0)
        ...
        def show_model_info():
            ...
            try:
                info = training_manager.load_model_info()
                print(f"  Modell-Name: {info.get('model_name', 'N/A')}")
                print(f"  Trainings-Version: {info.get('training_version', 'N/A')}")
                print(f"  Context Window: {info.get('context_window', 'N/A')}")
                print(f"  Max Response Tokens: {info.get('max_response_tokens', 'N/A')}")
                
                last_updated = info.get('last_updated', 0)
                if last_updated:
                    date_str = datetime.fromtimestamp(last_updated).strftime('%d.%m.%Y %H:%M')
                    print(f"  Letztes Update: {date_str}")
            except Exception as e:
                print(f"  Fehler: {str(e)}")
        ...
        def start_training_simulation():
            """Simuliert ein neues Training (für Demo-Zwecke)"""
            print_header()
            print("\n  TRAINING SIMULATION")
            print("  " + "-"*40)
            
            try:
                print("  Dies ist eine Demo-Funktion.")
                print("  In der echten Anwendung würde hier das Training")
                print("  mit neuen Daten gestartet werden.\n")
                
                # Simuliere Fortschritt
                for i in range(0, 101, 20):
                    print(f"  Fortschritt: {i}%", end='\r')
                    time.sleep(0.3)
                
                print("\n  Training abgeschlossen! (Simulation)")
                
                # Speichere eine Beispiel-Interaktion
                training_manager.save_training_interaction(
                    query="Beispiel-Training-Query",
                    context_results=[],
                    ai_response="Dies ist eine simulierte Antwort für das Training.",
                    feedback="positive"
                )
                print("  Beispiel-Interaktion gespeichert.")
        ...
        def run_cli_menu():
            ...
            while True:
                ...
                try:
                    ...
                    if choice == '1':
                        print("\n  Starte Web-Server...")
                        return True  # Signal zum Starten des Servers
                    elif choice == '2':
