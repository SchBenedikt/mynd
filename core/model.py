...
def select_model(force=None):
    ...
    try:
        ...
        raw = [m['name'] for m in r.json().get('models', []) if not any(b in m['name'] for b in LLM_BLACKLIST)]
    except (requests.RequestException, KeyError, TypeError, ValueError):
        pass
    ...
    try:
        choice = int(
            (
                _PROMPT.ask(f'{C.BOLD}Modell wählen{C.RESET}', default='1')
                if RICH
