...
def _write_memory(data):
    ...
    fd, temporary_name = tempfile.mkstemp(prefix=f'.{MEMORY_FILE.name}.', dir=MEMORY_FILE.parent)
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as handle:
            json.dump(data, handle, indent=2, ensure_ascii=False)
            handle.flush()
            os.fsync(handle.fileno())
        os.replace(temporary_name, MEMORY_FILE)
    finally:
...
def execute_ssh(host='', command='', user='', port=22, key='', password='', profile=''):
    ...
    try:
        if profile:
            base = f'vm/{profile}'
            if not host:
                host = _vault_get(f'{base}/ip')
            if not user:
                user = _vault_get(f'{base}/user') or 'root'
            if not password:
                password = _vault_get(f'{base}/password')
            if not key:
                key = _vault_get(f'{base}/key')
            if port == 22:
            ...
            if not host:
                host = _vault_get('vm/ip')
            if not user:
                user = _vault_get('vm/user') or 'root'
            if not password:
                password = _vault_get('vm/password')
            if not key:
                key = _vault_get('vm/key')
        port = int(port)
        ...
        if key and password and r.returncode != 0:
            ...
            r = subprocess.run(
                password_cmd, capture_output=True, text=True, timeout=60, env=ssh_env, input=password + '\n'
            )
...
def search_documents(query, top_k=10):
    try:
        ...
        embs = np.load(EMBS)
        qe = embed([query])[0]
        scores = np.array([float(np.dot(qe, e) / (np.linalg.norm(qe) * np.linalg.norm(e) + 1e-10)) for e in embs])
        top = np.argsort(scores)[-top_k:][::-1]
        parts = []
        for i in top:
            if scores[i] > 0.1:
                parts.append(f'[{chunks[i]["source"]}] (Score: {scores[i]:.2f})\n{chunks[i]["text"][:300]}')
...
def _validate_http_url(url):
    ...
    allow_private = {
        item.strip().lower() for item in os.getenv('MYND_HTTP_ALLOW_PRIVATE_HOSTS', '').split(',') if item.strip()
    }
...
def _news_queries_for_category(category):
    if category == 'technologie':
        return ['Technologie Neuigkeiten heute', 'KI News heute', 'IT Security News heute']
    return ['Neuigkeiten heute Deutschland', 'Wichtige Nachrichten heute', 'Aktuelle Welt Nachrichten heute']
...
def analyze_performance(tool_name=''):
    try:
        ...
        for name, s in sorted(stats.items(), key=lambda x: x[1]['success_rate']):
            ...
            lines.append(
                f'  {bar} {name}: {s["success_rate"]:.0f}% ({s["calls"]}x, Ø {s["avg_ms"]:.0f}ms, max {s["max_ms"]}ms)'
            )
        return '\n'.join(lines)
    except Exception:
...
CORE_TOOLS = [
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    'code': {
                        'type': 'string',
                        'description': 'Python-Code (KEIN input() oder interaktive Aufrufe). Ergebnis per print() ausgeben.',
                    }
                },
                'required': ['code'],
            },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'profile': {
                        'type': 'string',
                        'description': 'VM-Profilname für Vault-Prefix vm/<profil>/ (optional)',
                    },
                },
                'required': ['command'],
            },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    'category': {
                        'type': 'string',
                        'description': "Kategorie: 'top' (alle News) oder 'technologie' (Tech-News)",
                        'default': 'top',
                    },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    'thought': {
                        'type': 'string',
                        'description': 'Deine Überlegung – bei komplexen Aufgaben als Liste von Schritten pro Zeile formatieren (wird automatisch zu create_plan())',
                    },
                    'auto_plan': {
                        'type': 'boolean',
                        'description': 'True erzwingt create_plan(), auch bei einfachen Gedanken',
                    },
                },
                'required': ['thought'],
            },
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {
                    'key': {'type': 'string', 'description': "z.B. 'truenas/ip' – leer lassen für alle Keys"}
                },
                'required': [],
            },
        },
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {'key': {'type': 'string', 'description': 'Schlüssel'}},
                'required': ['key'],
            },
        },
    },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'max_results': {
                        'type': 'integer',
                        'description': 'Anzahl Bilder (default 6, max 12)',
                        'default': 6,
                    },
                },
                'required': ['query'],
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'secret': {
                        'type': 'boolean',
                        'description': 'Wenn true: Passwort-Masking (Sternchen)',
                        'default': False,
                    },
                },
                'required': ['message'],
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    'key': {
                        'type': 'string',
                        'description': "Schlüssel (z.B. 'user/name', 'network/ip_range'). Leer = alle.",
                    }
                },
                'required': [],
            },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    'steps': {
                        'type': 'string',
                        'description': 'Schritte, einer pro Zeile, in der Reihenfolge der Ausführung',
                    },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    'action': {
                        'type': 'string',
                        'description': 'Aktion: goto, click, type, snapshot, screenshot, extract, back, scroll',
                    },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    'problem': {
                        'type': 'string',
                        'description': 'Das Problem oder die Frage, die analysiert werden soll',
                    },
                    'method': {
                        'type': 'string',
                        'description': "Methode: 'tot' (Tree-of-Thought, default) oder 'step' (Schritt-für-Schritt)",
                        'default': 'tot',
                    },
                    'branches': {
                        'type': 'integer',
                        'description': 'Anzahl Denkpfade bei tot-Methode (1-5, default 3)',
                        'default': 3,
                    },
                    'depth': {
                        'type': 'integer',
                        'description': 'Tiefe pro Pfad bei tot-Methode (1-5, default 3)',
                        'default': 3,
                    },
                    'steps': {
                        'type': 'string',
                        'description': 'Optional: eigene Schritte für step-Methode, einer pro Zeile',
                    },
                },
                'required': ['problem'],
            },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'max_recent': {
                        'type': 'integer',
                        'description': 'Anzahl der letzten Aufrufe zur Analyse (default 5)',
                        'default': 5,
                    },
                },
                'required': [],
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'description': {
                        'type': 'string',
                        'description': 'Beschreibung wofür der Skill gut ist und wann er eingesetzt werden sollte',
                    },
                    'steps': {
                        'type': 'string',
                        'description': "JSON-Array der Tool-Schritte: [{'tool': 'name', 'args': {...}}, ...]",
                    },
                    'tags': {
                        'type': 'string',
                        'description': 'Komma-getrennte Tags für bessere Auffindbarkeit (optional)',
                    },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'max_results': {
                        'type': 'integer',
                        'description': 'Maximale Anzahl Skills (default 5)',
                        'default': 5,
                    },
                },
                'required': ['context'],
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {'name': {'type': 'string', 'description': 'Name des zu löschenden Skills'}},
                'required': ['name'],
            },
        },
    },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'description': {
                        'type': 'string',
                        'description': 'Beschreibung für das LLM wann/wie das Tool genutzt wird',
                    },
                    'parameters': {
                        'type': 'object',
                        'description': "JSON-Schema der Parameter: {'param_name': {'type': 'string', 'description': '...'}}",
                    },
                    'code': {
                        'type': 'string',
                        'description': "Python-Code (function-body). Parameter-Namen müssen mit 'parameters' übereinstimmen. Importe: requests, json, datetime, re, math, Path. KEIN input(), subprocess, os.system, eval, exec.",
                    },
                },
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {'name': {'type': 'string', 'description': 'Name des zu löschenden Tools'}},
                'required': ['name'],
            },
        },
    },
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {'plan_id': {'type': 'string', 'description': 'Plan-ID (erhalten bei create_plan)'}},
                'required': ['plan_id'],
            },
        },
    },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    'status': {
                        'type': 'string',
                        'description': "Filter: 'active', 'completed', 'completed_with_errors' (optional)",
                    }
                },
                'required': [],
            },
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {'tool_name': {'type': 'string', 'description': 'Optional: Nur dieses Tool analysieren'}},
                'required': [],
            },
        },
    },
...
# Runtime-generated Python plugins execute inside the backend process. Keep the
# feature disabled until it has a process-level sandbox and a reviewed admin
# approval workflow.
if os.getenv('MYND_ENABLE_DYNAMIC_TOOLS') != '1':
    _dynamic_names = {'create_tool', 'delete_tool', 'list_created_tools'}
    CORE_TOOLS[:] = [tool for tool in CORE_TOOLS if tool.get('function', {}).get('name') not in _dynamic_names]
    for _dynamic_name in _dynamic_names:
        CORE_MAP.pop(_dynamic_name, None)
