...
- **Streaming Markdown** (`frontend/lib/streamingMarkdown.js` + `MessageList.js`): `prepareStreamingMarkdown()` "repariert" unvollständige Markdown-Fragmente während des Live-Streamings — schließt offene Code-Fences (` ``` `), balanciert unvollständige Inline-Marker (`**`, `__`, `~~`, ` `` `, `$`, `$$`, `*`, `_`) nur in der letzten Zeile, damit Fett/Kursiv/Code live erscheinen statt als roher Syntax; nur bei `msg.isStreaming` aktiv, finale Nachrichten unverändert
